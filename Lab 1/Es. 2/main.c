...
int main() {
    ...
    fscanf(fd, "%d", &nsos);
    // ciclo per ogni parola del sorgente.txt, prendo anche un hcar vicino in modo che quando c'è il '\n'
    ...
    while((fscanf(fs, "%s%c", parola, &c)) != -1){
        ...
        // nel caso in cui sono arrivato al fondo del fil dizionario senza trovare corrispondenza riazzero il buffer
